...
buildindex="pandoc -f markdown -t html -s --data-dir="./" --template=pandoc-index-format --lua-filter=md-to-html-links.lua --filter=pandoc-crossref --metadata-file=./metadata.yaml --no-highlight -o"
...
find src -mindepth 2 -type f -name '_contents.md' | xargs cat | $buildindex out/index.html
mv out/src/* out/
